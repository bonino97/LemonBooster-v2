...
async function ExecuteAlive(payload, client) {
    try {
        ...
        if (enumeration) {
            ...
            /* SINTAXIS DE CADA HERRAMIENTA */
            const httprobe = `cat ${this.allSubdomainsFile} | httprobe | tee -a ${auxNewAlivesFile}`;
...
async function ExecuteScreenshot(payload, client) {
    try {
        ...
        if (enumeration) {
            /* SINTAXIS */
            const aquatone = `cat ${this.allAlivesFile} | aquatone -out ${enumeration.Directory}`;
...
async function ExecuteSubdomainResponseCodes(payload, client) {
    try {
        ...
        if (enumeration) {
            ...
            const hakcheckurl = `cat ${this.allAlivesFile} | hakcheckurl | tee -a ${auxNewResponseCodesFile}`;
...
async function ExecuteWaybackurl(payload, client) {
    try {
        ...
        if (discovery) {
            ...
            /* SINTAXIS DE CADA HERRAMIENTA */
            const waybackurl = `cat ${this.allAlivesFile} | waybackurls | tee -a ${auxNewWaybackurlsFile}`;
...
async function ExecuteGoSpider(payload, client) {
    try {
        ...
        if (discovery) {
            ...
            const goSpider = `gospider -S ${this.allAlivesFile} -d 0 --sitemap -t 3 | tee -a ${auxNewGoSpiderFile}`;
...
async function ExecuteHakrawler(payload, client) {
    try {
        ...
        if (discovery) {
            ...
            /* SINTAXIS DE LA HERRAMIENTA */
            const hakrawler = `cat ${this.allAlivesFile} | hakrawler -depth 2 -plain | tee -a ${auxNewHakrawlerFile}`;
